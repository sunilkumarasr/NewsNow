...
import com.example.newsatnow.databinding.NavItemCatBinding
import com.example.newsatnow.view.ArticalDetailActivity


class NavCatAdapter(// List that holds every item to be displayed in RecyclerView
    var texts: ArrayList<InterestsCategories>
) : RecyclerView.Adapter<NavCatAdapter.ViewHolder?>() {
    // This function inflated the list_item and fits it into the Recycler View Widget
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int) = ViewHolder (
        NavItemCatBinding.inflate(LayoutInflater.from(parent.context), parent, false)
    )

    // This funciton binds the Content with the components of the Recycler View.
    override fun onBindViewHolder(holder: NavCatAdapter.ViewHolder, position: Int) {
    ...
    class ViewHolder(val binding: NavItemCatBinding) : RecyclerView.ViewHolder(binding.root) {
